...
sigo<-read.delim('d:\\Rcode\\data_r_m_last.txt')##input the returns data
...
write.csv(aaaa, 'd:\\Rcode_3\\by\\xzy_1\\uk_resudials.csv')

sigo_1<-read.csv('d:\\Rcode_3\\by\\xzy_1\\uk_resudials.csv')##input the redusuals of returns
...
######################################
o<-order(sigo_1$oil)
rd<-cbind(sigo_1$oil[o],sigo_1$uk[o])
x<-rd[,1]
y<-rd[,2]
...
rootf1<-function(v){1 - exp(-((-log(1 - u_u[i]))^delta + (-log(1 - v))^delta)^(1/delta)) * 
    (((-log(1 - u_u[i]))^delta + (-log(1 - v))^delta)^((1/delta) - 
        ...
        (1/(1 - u_u[i]))))))-tau}
vx<-uniroot(rootf1,c(0.00000001,0.99999999))###在求落在区间 内的根
v_x[i]<-vx$root
...
write.csv(dynamic, 'd:\\Rcode_3\\by\\xzy_1\\CoVaR\\uk.csv')
