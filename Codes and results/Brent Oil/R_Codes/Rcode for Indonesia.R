...
## We take the Indonesia for example
...
#plot(fit$Indonesia,which="ask")##
##############################

s_res<-residuals(fit$Indonesia)
g_res<-residuals(fit$oil)

##process of Returns of the Indonesia
spec_s = uspec
setfixed(spec_s)<-as.list(coef(fit$Indonesia))
filt_s = ugarchfilter(spec_s, sigo$Indonesia)
rrss<-cbind(s_res,sigma(filt_s),s_res/sigma(filt_s))
ssss1<-cbind(sigo$Indonesia,fitted(filt_s),s_res,sigma(filt_s),s_res/sigma(filt_s))
...
write.csv(aaaa, 'Indonesia_resudials.csv')

sigo_1<-read.csv('Indonesia_resudials.csv')##input the redusuals of returns
...
op <- par(mfrow=c(1,1), mgp=c(1.5, 0.8,0), mar= .1+c(3,3,2,1),
font.main=6,font.lab=6)
n<-length(sigo$Indonesia)
tt<-seq(1:length(sigo$Indonesia))
plot(tt,sigo$Indonesia, type = "n", main =paste('Daily Returns and Conditional Value-at-Risk\n',
    "(alpha=", 0.05, ")", sep = ""), ylab = '', xlab = '', xlim=c(1,n),
            ylim = c(-16, 15), 
           cex.main = 0.8,  cex.lab = 0.9, cex.axis = 0.8)
        points(sigo$Indonesia, pch = 19, col = 'gray',cex=0.5)
...
write.csv(dynamic, 'Indonesia.csv')
