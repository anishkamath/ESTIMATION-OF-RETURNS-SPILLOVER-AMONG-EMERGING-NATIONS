...
#plot(fit$China,which="ask")##
##############################

s_res<-residuals(fit$China)
g_res<-residuals(fit$oil)

##process of Returns of the China
spec_s = uspec
setfixed(spec_s)<-as.list(coef(fit$China))
filt_s = ugarchfilter(spec_s, sigo$China)
rrss<-cbind(s_res,sigma(filt_s),s_res/sigma(filt_s))
ssss1<-cbind(sigo$China,fitted(filt_s),s_res,sigma(filt_s),s_res/sigma(filt_s))
...
write.csv(aaaa, 'China_resudials.csv')

sigo_1<-read.csv('China_resudials.csv')##input the redusuals of returns
...
font.main=6,font.lab=6)
n<-length(sigo$China)
tt<-seq(1:length(sigo$China))
plot(tt,sigo$China, type = "n", main =paste('Daily Returns and Conditional Value-at-Risk\n',
    "(alpha=", 0.05, ")", sep = ""), ylab = '', xlab = '', xlim=c(1,n),
            ylim = c(-16, 15), 
           cex.main = 0.8,  cex.lab = 0.9, cex.axis = 0.8)
        points(sigo$China, pch = 19, col = 'gray',cex=0.5)
