...
#plot(fit$India,which="ask")##
##############################

s_res<-residuals(fit$India)
g_res<-residuals(fit$oil)

##process of Returns of the India
spec_s = uspec
setfixed(spec_s)<-as.list(coef(fit$India))
filt_s = ugarchfilter(spec_s, sigo$India)
rrss<-cbind(s_res,sigma(filt_s),s_res/sigma(filt_s))
ssss1<-cbind(sigo$India,fitted(filt_s),s_res,sigma(filt_s),s_res/sigma(filt_s))
...
write.csv(aaaa, 'India_resudials.csv')

sigo_1<-read.csv('India_resudials.csv')##input the redusuals of returns
...
font.main=6,font.lab=6)
n<-length(sigo$India)
tt<-seq(1:length(sigo$India))
plot(tt,sigo$India, type = "n", main =paste('Daily Returns and Conditional Value-at-Risk\n',
    "(alpha=", 0.05, ")", sep = ""), ylab = '', xlab = '', xlim=c(1,n),
            ylim = c(-16, 15), 
           cex.main = 0.8,  cex.lab = 0.9, cex.axis = 0.8)
        points(sigo$India, pch = 19, col = 'gray',cex=0.5)
