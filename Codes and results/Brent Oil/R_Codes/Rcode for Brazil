...
## We take the Brazil for example
...
#plot(fit$Brazil,which="ask")##
##############################

s_res<-residuals(fit$Brazil)
g_res<-residuals(fit$oil)

##process of Returns of the Brazil
spec_s = uspec
setfixed(spec_s)<-as.list(coef(fit$Brazil))
filt_s = ugarchfilter(spec_s, sigo$Brazil)
rrss<-cbind(s_res,sigma(filt_s),s_res/sigma(filt_s))
ssss1<-cbind(sigo$Brazil,fitted(filt_s),s_res,sigma(filt_s),s_res/sigma(filt_s))
...
write.csv(aaaa, 'Brazil_resudials.csv')

sigo_1<-read.csv('Brazil_resudials.csv')##input the redusuals of returns
...
op <- par(mfrow=c(1,1), mgp=c(1.5, 0.8,0), mar= .1+c(3,3,2,1),
font.main=6,font.lab=6)
n<-length(sigo$Brazil)
tt<-seq(1:length(sigo$Brazil))
plot(tt,sigo$Brazil, type = "n", main =paste('Daily Returns and Conditional Value-at-Risk\n',
    "(alpha=", 0.05, ")", sep = ""), ylab = '', xlab = '', xlim=c(1,n),
            ylim = c(-16, 15), 
           cex.main = 0.8,  cex.lab = 0.9, cex.axis = 0.8)
        points(sigo$Brazil, pch = 19, col = 'gray',cex=0.5)
