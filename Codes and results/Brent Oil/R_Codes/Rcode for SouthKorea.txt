...
## We take the SouthKorea for example
...
#plot(fit$SouthKorea,which="ask")##
##############################

s_res<-residuals(fit$SouthKorea)
g_res<-residuals(fit$oil)

##process of Returns of the SouthKorea
spec_s = uspec
setfixed(spec_s)<-as.list(coef(fit$SouthKorea))
filt_s = ugarchfilter(spec_s, sigo$SouthKorea)
rrss<-cbind(s_res,sigma(filt_s),s_res/sigma(filt_s))
ssss1<-cbind(sigo$SouthKorea,fitted(filt_s),s_res,sigma(filt_s),s_res/sigma(filt_s))
...
write.csv(aaaa, 'SouthKorea_resudials.csv')

sigo_1<-read.csv('SouthKorea_resudials.csv')##input the redusuals of returns
...
ylab='Returns of the SouthKorea')
...
op <- par(mfrow=c(1,1), mgp=c(1.5, 0.8,0), mar= .1+c(3,3,2,1),
font.main=6,font.lab=6)
n<-length(sigo$SouthKorea)
tt<-seq(1:length(sigo$SouthKorea))
plot(tt,sigo$SouthKorea, type = "n", main =paste('Daily Returns and Conditional Value-at-Risk\n',
    "(alpha=", 0.05, ")", sep = ""), ylab = '', xlab = '', xlim=c(1,n),
            ylim = c(-16, 15), 
           cex.main = 0.8,  cex.lab = 0.9, cex.axis = 0.8)
        points(sigo$SouthKorea, pch = 19, col = 'gray',cex=0.5)
...
write.csv(dynamic, 'SouthKorea.csv')
